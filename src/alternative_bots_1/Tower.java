...
public class Tower extends Robot {
    ...
    protected void tryUpgrade() throws GameActionException {
        ...
        boolean isDefense = base == UnitType.LEVEL_ONE_DEFENSE_TOWER;
        boolean isPaint   = base == UnitType.LEVEL_ONE_PAINT_TOWER;

        int threshold;
        if (isDefense) {
            threshold = Constants.CHIPS_TO_UPGRADE_DEFENSE; 
        } else if (isPaint) {
            threshold = Constants.CHIPS_TO_UPGRADE_PAINT;   
        } else {
            threshold = Constants.CHIPS_TO_UPGRADE_MONEY;   
        }

        int nearbyAllies = nearbySoldiers + nearbyMoppers + nearbySplashers;
        int minAllies = isDefense ? 2 : 3;

        if (currChips > threshold && nearbyAllies >= minAllies) {
            rc.upgradeTower(rc.getLocation());
        } else if (currChips > Constants.CHIPS_TO_UPGRADE_FORCE) {
            rc.upgradeTower(rc.getLocation());
        }
    }
    ...
    protected UnitType pickSpawnType() {
        ...
        if (mapData.defenseTowers >= Constants.MIN_DEFENSE_FOR_SPAWN_BOOST) {
            int mod = spawnCount % 5;
            if (currChips > Constants.CHIPS_SPAWN_FREELY) {
                if (mod < 3)      return UnitType.SOLDIER;
                else if (mod < 4) return UnitType.SPLASHER;
                else              return UnitType.MOPPER;
            } else {
                if (mod < 4)      return UnitType.SOLDIER;
                else              return UnitType.MOPPER;
            }
        }
